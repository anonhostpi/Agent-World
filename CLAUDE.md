...
| **The Tracker** | **GitHub Issues + gh CLI** | Hierarchical issue decomposition — Epics → Issues → Sub-issues. Every spec, test, and implementation maps to a tracked issue. |
| **The Adversary** | **Sarcasmotron** (see below) | Hyper-critical reviewer with zero patience. Reviews specs, tests, *and* implementation. Fresh context on every pass. |

**The Adversary — Available Implementations:**

The Adversary must use a *different* model or context from the Builder to ensure cognitive diversity. Any of the following may serve as the Adversary:

| Tool | Model Family | Invocation |
| --- | --- | --- |
| **copilot** | Claude Sonnet 4.6 | `copilot -p "<prompt>"` |
| **gemini** | Gemini 3.1 Pro | `gemini -m gemini-3.1-pro-preview -p "<prompt>" --sandbox false` |
| **ollama** | Qwen3 32B / 14B (local) | `ollama run qwen3:32b "<prompt>"` |
| **opencode** | Multi-backend | `opencode` (interactive, configure provider) |
| **Claude subagent** | Same family, fresh context | Builder spawns a subagent with adversarial prompt |

**Adversary invocation pattern:** The Adversary must be instructed to read `CLAUDE.md` for coding standards before reviewing. Example:

```
Read CLAUDE.md for the coding standards (specifically VSDD Phase 3 and Section VII).
Then review <file> as a hyper-critical adversary. Zero tolerance.
Every point must be a concrete flaw with file and line number.
```

**Notes:**
- `copilot` and `gemini` provide the strongest cognitive diversity from Claude (different model families).
- `ollama` is costless and air-gapped — good for high-volume passes.
- A Claude subagent is acceptable when external tools are unavailable, but provides weaker diversity since it shares the same model family as the Builder.

**Triggering Adversarial Reviews — CLI Reference:**

Each tool has different flags for non-interactive adversarial use. All commands should be run from the repo root.

**copilot** — File review:
```bash
# Single file review
copilot -p "Read CLAUDE.md for standards. Review src/foo.ts as VSDD Phase 3 adversary." --quiet

# With full file access and auto-approval
copilot -p "<prompt>" --allow-all --quiet

# Specific model or reasoning effort
copilot -p "<prompt>" --model claude-sonnet-4-6 --effort high
```

**gemini** — File review:
```bash
# Default model hits capacity errors; always specify model
gemini -m gemini-3.1-pro-preview -p "<prompt>" --sandbox false

# Alternative stable model
gemini -m gemini-2.5-pro -p "<prompt>" --sandbox false

# Read-only mode (no file edits, just analysis)
gemini -m gemini-3.1-pro-preview -p "<prompt>" --approval-mode plan
```

**ollama** — Local/costless review:
```bash
# Pipe via stdin (ollama cannot read files; large diffs exceed arg limits)
{ cat CLAUDE.md; echo "---"; echo "Review this:"; cat src/foo.ts; } | ollama run qwen3:14b

# qwen3:14b is the default — fast enough for single-file reviews on CPU
{ cat CLAUDE.md; echo "---"; git diff origin/private...HEAD -- src/foo.ts; } | ollama run qwen3:14b

# qwen3:32b is last resort — very slow on CPU (~20GB RAM, hours for large diffs)
# Avoid --think with 32b on CPU; thinking phase alone can take 30+ minutes
{ cat src/foo.ts; } | ollama run qwen3:32b

# TODO: try qwen3:32b without --think and qwen3:14b with --think in future cycles
```

**Claude subagent** (Builder-initiated):
```
The Builder spawns a subagent via the Agent tool with:
- Fresh context (no shared conversation history)
- Adversarial system prompt
- Target files read into the prompt
...
| Flow | Command |
| --- | --- |
| **Spec Review (1c)** | `copilot -p "Read CLAUDE.md. Review GitHub issue #N for ambiguity, missing edge cases, contradictions."` |
| **Code Review (3)** | `gemini -m gemini-3.1-pro-preview -p "Read CLAUDE.md. Review src/foo.ts. Zero tolerance." --sandbox false` |
| **Symbol Audit (VII)** | `gemini -m gemini-3.1-pro-preview -p "Read CLAUDE.md Section VII. Scan src/ for multi-word symbols." --sandbox false` |
| **PR Diff Review** | `copilot -p "Run git diff origin/main...HEAD. Apply VSDD Phase 3 and Section VII."` |
| **Regression Check (2)** | Run test suite; verify 4-Result Rule (new fail + regression pass → new pass + regression pass). |
| **Costless Sweep** | `{ cat CLAUDE.md; echo "---"; cat src/foo.ts; } \| ollama run qwen3:14b` |
...
**GitHub Integration:** Each spec maps to a GitHub Issue. Sub-issues are generated for each behavioral contract item, edge case, non-functional requirement, *and* each formally provable property. The provable properties get their own issue chain so their status is tracked independently from test coverage.
...
**Testing Before Committing:** The test suite is committed first — as many commits as needed. No non-test commit may be made on the branch until the Red Gate clears (new tests fail, regression tests pass). The branch is unlocked for implementation only after that.
...
| **Section VII** | No unexcused multi-word symbol violations remain. All camelCase/snake_case findings have been resolved (refactored, excepted, or documented). This is a hard requirement — not deferrable to a follow-up PR. |

**Maximum Viable Refinement** is reached when all five dimensions have converged. The software is considered **Zero-Slop** — every line of code traces to a spec requirement, is covered by a test, has survived adversarial scrutiny, the critical path is formally proven, and the symbol surface is clean.
...
```
Spec Requirement → Verification Property → GitHub Issue → Test Case → Implementation → Adversarial Review → Formal Proof
```
...
5. **Linear Accountability:** GitHub Issues ensure every spec item, test, and line of code has a corresponding tracked unit of work. Nothing slips through the cracks.
...
### **VII. Architectural Review — Multi-Word Symbol Analysis**
